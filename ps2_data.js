...
/*
async function testPSZData() {
	const data = new PS2Data({
	...
	console.log( await data.getItemOwners(339) );
	const items = await data.getItems();
	items.forEach(item => console.log(`${item.id},${item.name},${item.price}`));
	console.log( await data.getItemInfos(213) );
	console.log( await data.getPlayerWallet("STEAM_0:1:73530340") );
}
